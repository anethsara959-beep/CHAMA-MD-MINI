...
const config = {
  ...
  BOT_VERSION: '3.0.0V',
  OWNER_NAME: '𝗠𝗿.𝗰𝗵𝗮𝗺𝗶𝗻𝗱𝘂 𝗿𝗮𝗻𝘀𝗶𝗸𝗮',
...
case 'deletemenumber': {
 ...
 //=========
// 📦 Required package: npm install @xaviabot/fb-downloader axios
// Make sure loadUserConfigFromMongo() function exists in your project.
...
// Requires: axios, cheerio, jimp
// npm i axios cheerio jimp

const pendingModApk = new Map(); // keep short-lived state if needed elsewhere
...
END:VCARD`
    ...
    if (!query) {
      return await socket.sendMessage(sender, {
        text: '🚫 *Please provide a search term.*\n\nExample: .modapk Angry Birds Classic',
        buttons: [{ buttonId: `${config.PREFIX}menu`, buttonText: { displayText: '📋 MENU' }, type: 1 }]
      }, { quoted: shonux });
    }

    await socket.sendMessage(sender, { text: `*⏳ Searching for:* ${query}` }, { quoted: shonux });
    ...
    const results = data.data.slice(0, 8); // limit to first 8
    let listText = `🔎 *Search results for:* ${query}\n\n`;
    results.forEach((r, i) => {
      listText += `*${i+1}.* ${r.title}\n   🧑‍💻 ${r.developer || 'Unknown'} • ⭐ ${r.rating?.value ?? '-'} (${r.rating?.percentage ?? '-' }%)\n\n`;
    });
    listText += `➡️ Reply with the result number (e.g. "1") within 2 minutes to download. Reply 'cancel' to abort.`;

    // send first image + list (if available)
    try {
      if (results[0]?.image) {
        await socket.sendMessage(sender, {
        ...
        await socket.sendMessage(sender, { text: listText }, { quoted: shonux });
      }
    } catch (e) {
      await socket.sendMessage(sender, { text: listText }, { quoted: shonux });
    }

    // Set up one-time listener for replies from this chat only
    const chatId = sender; // sender is chat id
    // safety: if there's already a pending listener for this chat, clear it first
    if (pendingModApk.has(chatId)) {
      const prev = pendingModApk.get(chatId);
      clearTimeout(prev.timer);
      if (prev.off) prev.off(); // remove previous event listener if we stored it
      pendingModApk.delete(chatId);
    }

    // helper: create JPEG thumbnail buffer from url (Jimp)
    const createThumb = async (imgUrl, maxBytes = 100 * 1024) => {
      try {
        const imgResp = await axios.get(imgUrl, { responseType: 'arraybuffer', timeout: 15000 });
        const Jimp = require('jimp');
        let image = await Jimp.read(Buffer.from(imgResp.data));
        image.cover(640, 640); // make square base
        let quality = 80;
        ...
        while (out.length > maxBytes && w > 120) {
          w = Math.floor(w * 0.8);
          const resized = image.clone().resize(w, Jimp.AUTO);
          out = await resized.quality(quality).getBufferAsync(Jimp.MIME_JPEG);
        }
        return out;
      } catch (e) {
        console.warn('thumb error', e?.message || e);
        return null;
      }
    };
    ...
    const handler = async (m) => {
      try {
        // messages.upsert returns an array in m.messages
        const upsertMsgs = m.messages || (m.message ? [m] : []);
        for (const incoming of upsertMsgs) {
          // ignore our own messages & only process messages in the same chat
          ...
          if (/^cancel$/i.test(body)) {
            await socket.sendMessage(chatId, { text: '🛑 Operation cancelled.' }, { quoted: shonux });
            cleanup();
            return;
          }
          ...
          if (/^[1-9][0-9]*$/.test(body)) {
            ...
            if (!results[idx]) {
              await socket.sendMessage(chatId, { text: '*❌ Invalid selection number.*' }, { quoted: shonux });
              cleanup();
              return;
            }

            const picked = results[idx];
            await socket.sendMessage(chatId, { text: `*⏳ Fetching page for:* ${picked.title}` }, { quoted: shonux });

            // fetch the page & attempt to find .apk link
            try {
              const pageResp = await axios.get(picked.link, { timeout: 20000 });
              ...
              $('a[href]').each((i, el) => {
                ...
                if (!apkUrl && href && href.match(/\.apk(\?|$)/i)) {
                  apkUrl = href.startsWith('http') ? href : new URL(href, picked.link).toString();
                }
              });
              if (!apkUrl) {
                const m = html.match(/https?:\/\/[^\s'"]+\.apk[^\s'"]*/i);
                if (m) apkUrl = m[0];
              }
              if (!apkUrl) {
                const selectors = ['.download a', '#download a', '.btn-download', '.download-button a'];
                for (const sel of selectors) {
                  const el = $(sel).first();
                  const href = el.attr('href');
                  if (href && href.match(/\.apk/i)) {
                    apkUrl = href.startsWith('http') ? href : new URL(href, picked.link).toString();
                    break;
                  }
                }
              ...
              if (apkUrl) {
                ...
                let jpegThumb = null;
                if (picked.image) {
                  jpegThumb = await createThumb(picked.image);
                }

                try {
                  await socket.sendMessage(chatId, {
                    document: { url: apkUrl },
                    fileName: `${picked.title.replace(/[^a-z0-9_\-\. ]/ig, '').slice(0,80)}.apk`,
                    mimetype: 'application/vnd.android.package-archive',
                    caption: `📥 *${picked.title}*\n🧑‍💻 ${picked.developer || 'Unknown'}\n⭐ ${picked.rating?.value ?? '-'} (${picked.rating?.percentage ?? '-' }%)\n\nSource: ${picked.link}`,
                    jpegThumbnail: jpegThumb || undefined
                  }, { quoted: shonux });
                  cleanup();
                  return;
                } catch (sendErr) {
                  // If sending by url fails (hotlink blocked), fallback to download -> upload
                  try {
                    await socket.sendMessage(chatId, { text: '⚠️ Direct send failed — downloading to server and re-uploading...' }, { quoted: shonux });
                    const resp = await axios.get(apkUrl, { responseType: 'arraybuffer', timeout: 60000, maxContentLength: 50 * 1024 * 1024 });
                    ...
                    console.error('download+upload failed', dlErr);
                    await socket.sendMessage(chatId, { text: '❌ Failed to fetch or send the APK file. You can open the page manually:\n' + picked.link }, { quoted: shonux });
                    cleanup();
                    return;
                  }
                }
                ...
                // no apk url found
                const fallbackText = `⚠️ Could not find a direct .apk URL automatically.\n\n*${picked.title}*\n🧑‍💻 ${picked.developer || 'Unknown'}\n⭐ ${picked.rating?.value ?? '-'} (${picked.rating?.percentage ?? '-' }%)\n\nOpen the page to download manually:\n${picked.link}`;
                try {
                  if (picked.image) {
                    await socket.sendMessage(chatId, { image: { url: picked.image }, caption: fallbackText }, { quoted: shonux });
                  } else {
                    await socket.sendMessage(chatId, { text: fallbackText }, { quoted: shonux });
                  }
                } catch (e) {
                  await socket.sendMessage(chatId, { text: fallbackText }, { quoted: shonux });
                }
              ...
              console.error('page fetch error', pageErr?.message || pageErr);
              await socket.sendMessage(chatId, { text: `❌ Failed to fetch the page: ${picked.link}\nYou can open the page manually.` }, { quoted: shonux });
              cleanup();
              return;
            }
          } // end numeric branch
        } // end for each incoming message
      } catch (handlerErr) {
        console.error('reply handler error', handlerErr);
      }
    }; // end handler

    // helper cleanup to remove listener and timeout
    let offFn = null;
    const cleanup = () => {
      try {
        if (offFn) offFn(); // remove the event listener
      } catch (e) {}
      if (pendingModApk.has(chatId)) {
        clearTimeout(pendingModApk.get(chatId).timer);
        pendingModApk.delete(chatId);
      }
    };

    // attach the listener and store a way to remove it
    // different Baileys versions expose off differently; we add wrapper to call socket.ev.off
    const evHandler = (m) => handler(m); // wrapper so we can reference the same function when removing
    socket.ev.on('messages.upsert', evHandler);
    offFn = () => {
      try { socket.ev.off('messages.upsert', evHandler); } catch(e){ /* best-effort */ }
    };

    // store pending state with timer & off function
    const timer = setTimeout(async () => {
      try {
        await socket.sendMessage(chatId, { text: '⏳ Timeout — selection window expired.' }, { quoted: shonux });
      } catch(e){}
      cleanup();
    }, 2 * 60 * 1000); // 2 minutes

    pendingModApk.set(chatId, { results, timer, off: offFn });

  } catch (err) {
    console.error('modapk case error', err);
    await socket.sendMessage(sender, { text: '*❌ Internal Error while searching.*' }, { quoted: shonux });
  }
  break;
}
...
END:VCARD`
    ...
    // --- helper: fetch + create WhatsApp-friendly JPEG thumbnail (<100KB) ---
    const makeJpegThumbnail = async (imgUrl, maxBytes = 100 * 1024) => {
      try {
        const imgResp = await axios.get(imgUrl, { responseType: 'arraybuffer', timeout: 15000 });
        const buffer = Buffer.from(imgResp.data);

        // Use Jimp to resize & compress
        const image = await Jimp.read(buffer);

        // Ensure max width/height (preserve aspect)
        image.cover(640, 640); // crop/cover to square 640x640 - good thumbnail base

        // iterate decreasing quality until under maxBytes
        let quality = 80;
        let outBuf = await image.quality(quality).getBufferAsync(Jimp.MIME_JPEG);

        while (outBuf.length > maxBytes && quality > 20) {
          quality -= 10;
          outBuf = await image.quality(quality).getBufferAsync(Jimp.MIME_JPEG);
        }

        // final check: if still too big, scale down dimensions
        let width = image.getWidth();
        while (outBuf.length > maxBytes && width > 160) {
          width = Math.floor(width * 0.8);
          const resized = image.clone().resize(width, Jimp.AUTO);
          outBuf = await resized.quality(quality).getBufferAsync(Jimp.MIME_JPEG);
        }
    ...
    let jpegThumb = null;
    if (result.image) {
      jpegThumb = await makeJpegThumbnail(result.image);
    }

    // If you want the image to ALSO be sent as an image message (so user sees it large)
    try {
      if (result.image) {
        // send image first (not mandatory) — users often like to see the app icon/preview
        await socket.sendMessage(sender, {
      ...
      // ignore image-send errors and continue to send document with thumbnail
      console.warn('Sending image message failed:', e?.message || e);
    }

    // Finally send the APK as document with jpegThumbnail (if available)
    await socket.sendMessage(sender, {
...
case 'alive': {
  try {
    ...
    const text = `
🤖 *${botName}* is online!
👑 *Owner*: ${config.OWNER_NAME || '𝗠𝗿.𝗖𝗵𝗮𝗺𝗶𝗻𝗱𝘂'}
...
case 'menu': {
  ...
  try {
    ...
    const title = userCfg.botName || '💙 𝗖𝗛𝗔𝗠𝗔 𝗠𝗜𝗡𝗜 𝗕𝗢𝗧 💜';
...
│ ${config.PREFIX}alive
│
> © ${config.BOT_FOOTER || '𝗖𝗛𝗔𝗠𝗔 - 𝗠𝗜𝗡𝗜'}
`.trim();

    // Buttons: main actions + two quick links (pair & website)
    const buttons = [
      ...
      { buttonId: `${config.PREFIX}pair`, buttonText: { displayText: "🔗 PAIR" }, type: 1 }
    ];

    // Secondary template message (list-style) for devices that support it
    const listMessage = {
      title: `${title} — Quick Actions`,
      description: 'Tap a button or use the commands below',
      buttonText: 'Open Menu',
      listType: 1,
      sections: [
        { title: 'Quick Links', rows: [
            { title: '🔗 Pair device', description: 'Pair this number with the web panel', rowId: `${config.PREFIX}pair` },
            { title: '🌐 Open website', description: 'Open the mini web panel', rowId: 'open_website' }
          ]
        },
        { title: 'Main Commands', rows: [
            { title: '📥 Download', description: 'Download media (video/audio)', rowId: `${config.PREFIX}download` },
            { title: '🎨 Creative', description: 'Image tools & AI', rowId: `${config.PREFIX}creative` }
          ]
        }
      ]
    };
    ...
    // Send image + buttons (fallback to list message for capable clients)
    try {
      await socket.sendMessage(sender, {
        image: imagePayload,
        caption: text,
      ...
      // also send list message for richer clients (non-blocking)
      try { await socket.sendMessage(sender, listMessage, { quoted: msg }); } catch(e){}

    } catch (e) {
      // fallback: plain text if rich message fails
...
END:VCARD` } }
    ...
    break;
// 'ai', 'gemini', 'ask' case එක වෙනුවට 'math' හෝ 'solve' case එක භාවිතා කළ හැකිය.
case 'math':
case 'solve':
    try {
        if (!args || args.length === 0) {
            await socket.sendMessage(sender, {
                text: '🔢 *Please provide the equation or math problem.*\n\n*Example:*\n.math solve dy/dx = (x^2+y^2)/(2xy) step-by-step'
            });
            break;
        }
        ...
        // ⚠️ අලුත් API Key එක මෙතනට යොදන්න (රහසිගතව තබාගන්න!)
        const geminiApiKey = "AIzaSyAEPKi6PDU_Ai8ROpBBYxMLuLdvrJEIRG8"; 
        const apiUrl = `https://generativelanguage.googleapis.com/v1beta/models/gemini-pro:generateContent?key=${geminiApiKey}`;

        // 💡 ගණිතය සඳහා විශේෂිත උපදෙස් (System Instruction)
        const mathInstruction = `
            You are a specialized Math Solver Bot. Your primary function is to solve mathematical problems, including differential equations, algebra, and calculus.
            ALWAYS provide a step-by-step solution. 
            Format the mathematical parts using LaTeX syntax for clarity.
            For inline math, use $...$. For block equations, use $$...$$.
            The final answer must be clear and distinct.
        `;

        // Gemini API එකට යැවිය යුතු දත්ත (payload)
        const requestData = {
            // System Instruction එක භාවිතයෙන් AI ගේ හැසිරීම පාලනය කිරීම
            config: {
                systemInstruction: mathInstruction 
            },
            contents: [{
                parts: [{
                    text: `Solve the following problem step-by-step and provide the answer in LaTeX/text format: ${query}`
                }]
            }]
        };
        ...
        if (response.data.candidates && response.data.candidates.length > 0 &&
            ...
            await socket.sendMessage(sender, { text: `⚠️ *AI did not provide a solution for:* ${query}` });
            break;
        }

        // --- WhatsApp පිළිතුර සකස් කිරීම ---
        
        const botName = "Math Solver AI";
        const botMention = { 
            // මෙහිදී ඔබගේ botMention code එක නැවත භාවිතා කළ හැකිය.
            key: { remoteJid: "status@broadcast", participant: "0@s.whatsapp.net", fromMe: false, id: "MATH_AI_FAKE_ID" },
            message: { contactMessage: { displayName: botName, vcard: `BEGIN:VCARD\nVERSION:3.0\nFN:${botName}\nORG:Google AI;\nEND:VCARD` } }
        };

        const formattedResponse = `🧠 *Gemini Math Solver for:* "${query}"\n\n${aiResponse.trim()}`;
        
        // ගණිතමය පිළිතුරු සඳහා වෙනස් thumbnail එකක් භාවිතා කිරීම
        const thumbnailUrl = 'https://i.ibb.co/qMpzYDF9/tourl-1762411871876.jpg'; // ගණිත icon එකක් වැනි.
        ...
        console.error(`Gemini Math Solver error:`, error.response ? error.response.data : error.message);
        let errorMessage = error.message;
        if (error.response && error.response.data && error.response.data.error) {
            errorMessage = error.response.data.error.message;
        }
        await socket.sendMessage(sender, { text: `⚠️ *An error occurred while solving the problem.*\n\nError: ${errorMessage}` });
    }
    break;
case 'gemini':
case 'ais':
    try {
        // User query එකක් ලබා දී නැත්නම් error එකක් යැවීම
        if (!args || args.length === 0) {
            await socket.sendMessage(sender, {
                text: '⚠️ *Please provide a question for the AI.*\n\n*Example:*\n.ai what is the capital of Sri Lanka'
            });
            break;
        }
        ...
        // ⚠️ මෙතැනට ඔබේ Gemini API Key එක යොදන්න
        // Google AI Studio වෙතින් මෙය නොමිලේ ලබාගත හැක.
        const geminiApiKey = "AIzaSyAEPKi6PDU_Ai8ROpBBYxMLuLdvrJEIRG8"; 
        ...
        // axios.post request එකක් මගින් API එකට කතා කිරීම
        const response = await axios.post(apiUrl, requestData, {
            headers: {
                'Content-Type': 'application/json'
            }
        ...
        // AI එකෙන් ලැබුණු පිළිතුර (text) නිවැරදිව ලබා ගැනීම
        let aiResponse = '';
        if (response.data.candidates && response.data.candidates.length > 0 &&
            response.data.candidates[0].content && response.data.candidates[0].content.parts &&
            response.data.candidates[0].content.parts.length > 0) {
            ...
            // AI එකෙන් පිළිතුරක් නොලැබුනහොත්
            await socket.sendMessage(sender, { text: `⚠️ *AI did not provide a valid response for:* ${query}` });
            break;
        }

        // --- Bot එකේ පිළිතුර සකස් කිරීම ---
        
        // (ඔබේ google search එකේ තිබූ botMention එකම මෙහි භාවිතා කළ හැක)
        const botName = "Google AI"; // ඔබට අවශ්‍ය නමක් දෙන්න
        const botMention = {
            key: { remoteJid: "status@broadcast", participant: "0@s.whatsapp.net", fromMe: false, id: "META_AI_FAKE_ID_GEMINI" },
            message: { contactMessage: { displayName: botName, vcard: `BEGIN:VCARD\nVERSION:3.0\nFN:${botName}\nORG:Google AI;\nEND:VCARD` } }
        };

        // AI පිළිතුර format කිරීම
        const formattedResponse = `🤖 *Google AI Response:*\n\n${aiResponse.trim()}`;
        
        // AI පිළිතුර සමග thumbnail එකක් යැවීම (ඔබේ පැරණි code එකේ මෙන්)
        const thumbnailUrl = 'https://i.imgur.com/R3zYyUa.png'; // Google AI සඳහා වන සාමාන්‍ය icon එකක්
        ...
        await socket.sendMessage(sender, { text: `⚠️ *An error occurred while fetching the AI response.*\n\n${errorMessage}` });
    }
    break;
// මෙතනින් case එක අවසන් වේ
case 'img': {
...
END:VCARD` } }
        ...
        const res = await axios.get(`https://api.siputzx.my.id/api/images?query=${encodeURIComponent(q)}`);
...
case 'web': {
  try {
    ...
    const template = {
      image: { url: img },
      caption: `✦ ${BOT_NAME_FANCY} ✦\n\n🔗 Tap below to open the web panel:`,
      footer: config.BOT_FOOTER || BOT_NAME_FANCY,
      templateButtons: [
        { urlButton: { displayText: '🔗 Open Website', url } },
        { quickReplyButton: { displayText: '📋 MENU', id: `${config.PREFIX}menu` } }
      ]
    };

    await socket.sendMessage(sender, template, { quoted: msg });
  } catch (e) {
...
case 'csong': {
  try {
    ...
    const caption = `☘️ ᴛɪᴛʟᴇ : ${title}

❒ *🎭 Vɪᴇᴡꜱ :* ${apiRes.views || 'N/A'}
❒ *⏱️ Dᴜʀᴀᴛɪᴏɴ :* ${duration}
❒ *📅 Rᴇʟᴇᴀꜱᴇ Dᴀᴛᴇ :* ${apiRes.release || apiRes.uploadDate || 'N/A'}
